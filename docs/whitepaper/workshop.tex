...
The code verification pipeline correctly classifies clean code as
\textsc{passed} (confidence $\geq 0.7$), unsafe code (\texttt{os},
\texttt{eval}) as \textsc{failed} with appropriate security risk levels,
and syntactically invalid code as \textsc{failed} with descriptive error
messages.  All 10 security rules detect their target vulnerability
classes with zero false negatives on the test corpus.  The sandbox
successfully blocks all 10 dangerous module imports.
