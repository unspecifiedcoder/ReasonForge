...
\begin{tikzpicture}[
  ...
  % Output
  \node[out, below=0.8cm of circuit] (proof) {Groth16 Proof (128--256 B)};
...
\subsection{Code Verification Accuracy}

The code verification pipeline correctly classifies three categories of
input:

\begin{enumerate}[nosep]
  \item \textbf{Clean code} (well-typed, None-safe functions with
    docstring examples): classified as \textsc{passed} with confidence
    scores $\geq 0.7$.

  \item \textbf{Unsafe code} (imports of \texttt{os},
    \texttt{subprocess}, or use of \texttt{eval}/\texttt{exec}):
    classified as \textsc{failed} with security risk levels of ``high''
    or ``critical'' and at least one failure reason identifying the
    security violation.

  \item \textbf{Malformed code} (syntax errors): classified as
    \textsc{failed} with \texttt{syntax\_valid = false}, a descriptive
    syntax error message, zero tests generated, and zero tests executed.
\end{enumerate}

The integration test suite verifies all three classification paths
end-to-end, including certificate creation, registry registration, and
look-up.

\subsection{Security Scanner Coverage}

All 10 AST-based detection rules were individually tested with targeted
code samples.  Each rule correctly identifies the vulnerability class,
assigns the expected severity level, and reports the correct line number.
The scanner handles syntactically invalid input gracefully by returning a
``critical'' risk assessment with a \texttt{RF-SEC-SYNTAX} pseudo-rule.
When multiple vulnerabilities are present, the overall risk level is
correctly set to the highest severity found.

\subsection{Sandbox Isolation}

The sandbox executor successfully blocks all 10 dangerous module imports.
Import attempts for \texttt{os}, \texttt{subprocess}, and other blocked
modules result in non-zero exit codes and error messages containing
``blocked'' or ``import'' keywords.  Safe imports (\texttt{math},
\texttt{json}, standard library modules not on the block list) execute
normally.  The test execution mode (\texttt{execute\_with\_tests})
correctly runs user-provided assertions against the sandboxed code and
reports pass/fail outcomes.

\subsection{End-to-End Pipeline Performance}

The full test suite of 316 tests executes in approximately 12~seconds,
with the code verification integration tests (which invoke real
subprocess sandboxed execution) completing in under 10~seconds each.
The pipeline produces deterministic code hashes for identical inputs
and distinct hashes for different inputs, ensuring certificate
uniqueness.  The certificate round-trip---pipeline verification
$\rightarrow$ report $\rightarrow$ certificate creation $\rightarrow$
registry registration $\rightarrow$ look-up $\rightarrow$
